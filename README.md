...
## 🏃 프로젝트 소개
웹 개발에 대해 공부한 뒤로 처음 경험해 보는 프로젝트였기에 프로젝트 주제에 대해 고민이 많았습니다.
...
- 상품을 카테고리를 세분화하기 위해 카테고리는 자기 참조를 통해 상위 카테고리와 하위 카테고리를 갖습니다.
- 회원과 관리자, 상품은 삭제 시 연관된 데이터들도 같이 삭제되어야 하기 때문에 삭제 여부 Column을 통해 삭제 처리했습니다.
- 결제 상태와 성별을 Enum 타입으로 관리하여 가독성과 유지 보수성을 향상시켰습니다.
...
- 회원의 상품 조회와 주문 내역 조회, 관리자의 회원 조회와 같은 조회 API에서 조건부 필터를 통해 페이징 된 다양한 결과를 얻을 수 있습니다.
...
- 로그인하고 발급받은 Token에 저장된 권한을 바탕으로 API에 대한 인가 처리를 진행합니다.
- 회원은 리뷰, 문의, 마이페이지, 검색, 결제 서비스를 구현하였으며, 회원가입 / 로그인 및 조회는 인가 과정을 생략하도록 구현하였습니다.
...
- 이미지는 클라이언트에게 보일 순서를 지정할 수 있으며, 순서가 0인 메인 이미지는 대표 이미지로써 사용자에게 보입니다.
...
- 신용카드, 휴대전화, 가상 계좌 등 다양한 결제 방법을 지원하기 때문에 확장성이 좋습니다.
- 실시간 결제 상태를 모니터링할 수 있는 기능을 제공하기 때문에 관리에 용이합니다.
...
### 🍑 느낀점
- 보안 취약점을 직접 발견하고 해결하면서 검증의 중요성을 다시 한번 깨달았습니다.
...
### 🍓 문제 배경
- 상품을 검색했을 때 상품의 여러 이미지 중 어떤 이미지를 사용자에게 보이게 할지 고민하였습니다.
- 이미지 중 하나를 사용자에게 보여줘도 되지만, 대표 이미지를 지정하고 이미지의 순서를 조정하는 기능에 대한 필요성을 느꼈습니다.

### 🍓 해결 방법
- 상품에 이미지를 등록할 때 순서가 정해지며, 순서가 0인 이미지가 대표 이미지로써 상품 목록에서 사용자에게 보입니다.
- 상품 상세 페이지에서는 메인 이미지, 서브 이미지가 순서에 맞게 사용자에게 보입니다.
